...
export function Article({ item }: ArticleProps) {
    ...
    return (
        <Link key={item.id} to={`/item/${item.id}`} className="block h-fit">
            <article className="relative bg-card border border-border rounded-lg hover:shadow-md transition-shadow cursor-pointer flex flex-col ">
                {item.hasVideo && <div className="absolute -top-1 -right-1 rounded-4xl p-2 bg-background/85"><LucideVideo strokeWidth={.6} /></div>}
                {item.imageUrl && (
                    <img
                        src={item.imageUrl}
                        alt={item.title}
                ...
                <section className="p-4">
                    <h2 className="text-lg font-semibold text-foreground mb-2 line-clamp-2">{item.title}</h2>
                    <p className="text-sm text-muted-foreground line-clamp-3 mb-2">{item.abstract}</p>
                    <div className="text-xs text-muted-foreground space-y-1">
                        ...
                        <div>Status: {item.status} | Created: {new Date(item.created_at).toLocaleDateString()}</div>
                        <div>Views: {item.views} | Downloads: {item.downloads}</div>
